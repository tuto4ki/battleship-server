...
export function attack(
  ...
  const player = roomsCurrent.usersID.find(
  ...
  if (player?.attackMatrix[data.x][data.y]) {
    sendMessageAttack(
      ...
      roomsCurrent,
      usersDB,
      getEnumStatusAttack(player?.attackMatrix[data.x][data.y]),
    );
    return;
  }

  const enemy = roomsCurrent.usersID.find(
    (user) => user.index !== data.indexPlayer,
  );
  if (enemy && player) {
    const statusAttack = getStatusAttack(
      { x: data.x, y: data.y },
      enemy.shipsMatrix,
      enemy.ships,
    );
    const turnIndexAttack =
      statusAttack === EShotType.miss ? enemy.index : data.indexPlayer;
    ...
    if (
      (roomsCurrent.currentPlayer === enemy.index &&
        !usersDB.get(enemy.index)?.ws) ||
      (roomsCurrent.currentPlayer === player.index &&
        !usersDB.get(player.index)?.ws)
    ) {
      randomAttack(
        {
          gameId: roomsCurrent.indexRoom,
          indexPlayer: roomsCurrent.currentPlayer,
        },
        roomsCurrent,
        usersDB,
        winsDB,
...
function sendMessageAttack(
  ...
  for (let k = 0; k < killCell.length; k++) {
    player.attackMatrix[killCell[k].x][killCell[k].y] =
      getNumberStatusAttack(status);
...
export function randomAttack(
  ...
  if (player) {
    ...
    attack(
      {
        ...data,
        x: randomShotMatrix[randomCell][0],
        y: randomShotMatrix[randomCell][1],
      },
      roomsCurrent,
      usersDB,
      winsDB,
...
function getStatusAttack(
  posPlayer: TPosition,
  enemyShips: Array<Array<number>>,
  enemyMatrix: TCell[],
): EShotType {
